...
import os
# scalling not possible because all of our data is in log(x)?
# error bars --> done 
# add points --> done
# stitch files --> done
# fix invert axis --> done
# review the logic of code, what is it doing and does it make sense
# any other language commonly used by astronmers
    ...
    while decision != '6':
        if isint(decision) != True or int(decision) > 6 or int(decision) < 1:
            ...
            while point == 'yes':
                ...
                x_point_err = input("Please provide the error for ffective temperature in Kelvin: Log(T_eff) =  ")
                ...
                y_point_err = input("Please provide the bolometric luminosity of the star in: Log(L) = ")
                while isint(y_point_err) == False and isfloat(y_point_err) == False:
                    y_point_err = input("Invalid input. Please provide the effective temperature in Kelvin: Log(T_eff) =  ")
