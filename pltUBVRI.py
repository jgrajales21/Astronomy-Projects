...
def pltcmc():
    ...
    """
    uses iso files to create curves of constant mass and varying age
    """
    ...
    #MIST_v1.2_feh_m1.00_afe_p0.0_vvcrit0.0_UBVRIplus.iso
    #MIST_v1.2_feh_m4.00_afe_p0.0_vvcrit0.0_UBVRIplus.iso
    #MIST_v1.2_feh_m1.75_afe_p0.0_vvcrit0.0_UBVRIplus.iso
    #MIST_v1.2_feh_m2.00_afe_p0.0_vvcrit0.0_UBVRIplus.iso
    print(cwd + untar_iso_path[-30:]+"/MIST_v1.2_feh_"+index+"_afe_p0.0_vvcrit0.0_UBVRIplus.iso.cmd")
    datai = ascii.read(cwd + untar_iso_path[-30:]+"/MIST_v1.2_feh_"+index+"_afe_p0.0_vvcrit0.0_UBVRIplus.iso.cmd")
    ...
    #max_age = agei[len(agei)-1] #10 Gyr in each file 
    max_age = np.amax(agei)
    min_age = np.amin(agei)
    #min_age = agei[0]           # 100,000 years in each file 

    #mass ranges in solar masses
    #max_mass = massi[len(massi)-1]
    max_mass = np.amax(massi)
    min_mass = np.amin(massi)
    #min_mass = massi[0]
    ...
    while user_dec != 'no':
        ...
        if user_dec == 'yes':
            ...
            user_age_max = float(input("Enter maximum age, in log years, for constant mass curve (range: "+str(min_age)+" (100,000 years) ~ "+str(max_age)+ " (10 Gyr)): "))
            j = False
            while j != True:
                if user_age_max > max_age or user_age_max < min_age:
                    print("Invalid entry, try again.")
                    user_age_max = float(input("Enter maximum age, in log years, for constant mass curve (range: "+str(min_age)+" (100,000 years) ~ "+str(max_age)+ " (10 Gyr)): "))
                    j = False
                else:
            ...
            #plot curve
            i1 = plt.plot(logti[loci1], logli[loci1], color = 'red', linestyle = 'dashed')

            
            user_dec = input("Would you like to plot another mass curve? Enter 'no' to terminate: ")
...
#implement interpolation scheme to highlight ranges of star massess in evolutionary tracks
